...
delete_headers = {"Authorization": f"Bearer {login_user_token}"}
delete_user = httpx.delete(f"http://localhost:8000/api/v1/users/{create_user_id}", headers=delete_headers)
delete_user_data = delete_user.json()
print(f"если 200 то пользователь удален: {delete_user.status_code}")
